...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    activation_all = x@Wx + prev_h@Wh + b #N*4H
    ig = sigmoid(activation_all[:, :H])
    fg = sigmoid(activation_all[:, H:2*H])
    og = sigmoid(activation_all[:, 2*H:3*H])
    g = np.tanh(activation_all[:, 3*H:4*H])
    next_c = fg * prev_c + ig * g
    next_h = og * np.tanh(next_c)
    cache = (x, prev_h, prev_c, Wx, Wh, b, activation_all, ig, fg, og, g, next_c, next_h)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, prev_h, prev_c, Wx, Wh, b, activation_all, ig, fg, og, g, next_c, next_h = cache
    dog = dnext_h * np.tanh(next_c)
    dnext_c += dnext_h *og * (1-np.tanh(next_c)**2)
    a1, a2, a3, a4 = np.split(activation_all, 4, axis=1)
    da4 = dog * sigmoid(a4) * (1-sigmoid(a4))
    dig = dnext_c * g
    dg = dnext_c * ig
    da3 = dg * (1-np.tanh(a3)**2)
    da2 = dig * sigmoid(a2)*(1-sigmoid(a2))
    dprev_c = dnext_c * fg
    dfg = dnext_c * prev_c
    da1 = dfg * sigmoid(a1)*(1-sigmoid(a1))
