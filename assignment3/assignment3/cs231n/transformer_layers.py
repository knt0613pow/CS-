...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        i = torch.arange(max_len).view([-1, 1]).float()
        j = torch.arange(0, embed_dim, 2)
        ex = torch.exp((- math.log(10000)/ embed_dim)*j).view([1,-1])
        pe[:, :, 0::2] = torch.sin(i.matmul(ex))
        pe[:, :, 1::2] = torch.cos(i.matmul(ex))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        주어진 데이터에 대해 attention 을 계산하자
        N: batch size
        S : Source sequence length
        T: target seqeunce length
        ...
        Q = self.query(query).view(N,S, H, HD).permute(0,2,1,3) # N,H, S, HD
        K = self.key(key).view(N, T, H, HD).permute(0,2,1,3)
        V = self.value(value).view(N,T, H, HD).permute(0,2,1,3)
