...
def compute_saliency_maps(X, y, model):
    ...
    # torch.max method는 max matrix와 index 두가지를 tuple 형태로 out 하므로 [0]을 붙여야 함
...
def make_fooling_image(X, target_y, model):
    ...
    for i in range(100):
        ...
        max_class = torch.max(scores, 1)[1]
        if target_y == max_class:
            break
        target_score = scores[0, target_y] # scores의 shape 는 (1, 1000) 이므로
